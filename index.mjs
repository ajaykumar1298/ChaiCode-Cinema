...
// seats demo data added
const count = await seatModel.countDocuments();
if (count === 0) {
  for (let i = 1; i <= 20; i++) {
    await seatModel.create({ id: i });
  }
...
app.put("/:id/:name", authUser, async (req, res) => {
  // try {
  ...
  try {
    ...
    const isSeatExist = await seatModel.findOne({
      $or: [{ id: seatId }],
    });
    console.log(isSeatExist);
    if (!isSeatExist) {
      return res.status(401).json({
        msg: "seat number is not exist",
      });
    }
